...
# -----------------------------------------------------------------------------
# Helper: find HLS IP repository folder (must contain component.xml)
# -----------------------------------------------------------------------------
proc find_hls_ip_repo {} {
    set candidates [list \
        "./hls_gray_ip/hls_export/myip" \
    ]

    foreach c $candidates {
        set abs_c [file normalize $c]
        if {![file exists $abs_c]} {
            continue
        }

        # Case 1: component.xml is directly in this folder
        if {[file exists [file join $abs_c component.xml]]} {
            return $abs_c
        }

        # Case 2: component.xml is one level below
        set subdirs [glob -nocomplain -directory $abs_c *]
        foreach s $subdirs {
            if {[file isdirectory $s] && [file exists [file join $s component.xml]]} {
...
set hls_ip_repo [find_hls_ip_repo]
if {$hls_ip_repo ne ""} {
    puts "INFO: Found HLS IP repository at: $hls_ip_repo"
    lappend ip_repos $hls_ip_repo
} else {
    puts "WARNING: HLS IP repository not found."
    puts "WARNING: Expected path like ./hls_gray_ip/hls_export/myip containing component.xml"
}
