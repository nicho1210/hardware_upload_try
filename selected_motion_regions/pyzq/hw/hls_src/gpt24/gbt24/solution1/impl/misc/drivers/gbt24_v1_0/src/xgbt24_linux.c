...
typedef struct {
    ...
    XGbt24_uio_map maps[ MAX_UIO_MAPS ];
} XGbt24_uio_info;

/***************** Variable Definitions **************************************/
static XGbt24_uio_info uio_info;
...
static int uio_info_read_name(XGbt24_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/name", info->uio_num);
    return line_from_file(file, info->name);
}

static int uio_info_read_version(XGbt24_uio_info* info) {
    char file[ MAX_UIO_PATH_SIZE ];
    sprintf(file, "/sys/class/uio/uio%d/version", info->uio_num);
    return line_from_file(file, info->version);
}

static int uio_info_read_map_addr(XGbt24_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
static int uio_info_read_map_size(XGbt24_uio_info* info, int n) {
    int ret;
    char file[ MAX_UIO_PATH_SIZE ];
...
int XGbt24_Initialize(XGbt24 *InstancePtr, const char* InstanceName) {
	XGbt24_uio_info *InfoPtr = &uio_info;
