...
-- 0x2c : reserved
-- 0x30 : Data signal of enable_blur
--        bit 31~0 - enable_blur[31:0] (Read/Write)
-- 0x34 : reserved
-- (SC = Self Clear, COR = Clear on Read, TOW = Toggle on Write, COH = Clear on Handshake)

architecture behave of gbt24_CTRL_s_axi is
    ...
    constant ADDR_ENABLE_BLUR_DATA_0 : INTEGER := 16#30#;
    constant ADDR_ENABLE_BLUR_CTRL   : INTEGER := 16#34#;
    ...
    signal int_enable_blur     : UNSIGNED(31 downto 0) := (others => '0');
        ...
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (ar_hs = '1') then
                    ...
                    when ADDR_ENABLE_BLUR_DATA_0 =>
                        rdata_data <= RESIZE(int_enable_blur(31 downto 0), 32);
                    when others =>
                        NULL;
                    end case;
                end if;
...
-- ----------------------- Register logic ----------------
    ...
    enable_blur          <= STD_LOGIC_VECTOR(int_enable_blur);
    ...
    begin
        if (ACLK'event and ACLK = '1') then
            if (ACLK_EN = '1') then
                if (w_hs = '1' and waddr = ADDR_ENABLE_BLUR_DATA_0) then
                    int_enable_blur(31 downto 0) <= (UNSIGNED(WDATA(31 downto 0)) and wmask(31 downto 0)) or ((not wmask(31 downto 0)) and int_enable_blur(31 downto 0));
                end if;
            end if;
        end if;
    end process;
