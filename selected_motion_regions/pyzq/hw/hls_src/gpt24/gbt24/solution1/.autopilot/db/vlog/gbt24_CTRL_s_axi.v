...
localparam
    ...
    ADDR_ENABLE_BLUR_DATA_0 = 6'h30,
    ADDR_ENABLE_BLUR_CTRL   = 6'h34,
    WRIDLE                  = 2'd0,
    WRDATA                  = 2'd1,
    WRRESP                  = 2'd2,
    WRRESET                 = 2'd3,
    ...
    if (ACLK_EN) begin
        if (ar_hs) begin
            ...
            case (raddr)
                ...
                ADDR_ENABLE_BLUR_DATA_0: begin
                    rdata <= int_enable_blur[31:0];
                end
            endcase
        end
    end
...
assign enable_blur       = int_enable_blur;
// int_interrupt
always @(posedge ACLK) begin
    if (ARESET)
        int_interrupt <= 1'b0;
...
// int_enable_blur[31:0]
always @(posedge ACLK) begin
    if (ARESET)
        int_enable_blur[31:0] <= 0;
    else if (ACLK_EN) begin
        if (w_hs && waddr == ADDR_ENABLE_BLUR_DATA_0)
            int_enable_blur[31:0] <= (WDATA[31:0] & wmask) | (int_enable_blur[31:0] & ~wmask);
    end
end
